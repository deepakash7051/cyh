		...
		<div class="form-wrap">
			<form action="{{ route('admin.modules.update', $module->id) }}" method="POST" enctype="multipart/form-data">
	                    ...
	                    @if(count($courses) > 0)
	                        @foreach($courses as $course)
	                            <option value="{{$course->id}}" 
	                            		{{ $module->course_id == $course->id ? 'selected="selected"' : '' }}
	                            	>
	                                {{$course->$title}}
	                            </option>
	                        @endforeach
	            ...
	            @if(count($languages) > 0)
	                @foreach($languages as $langKey => $langValue)
	                    @php 
	                        ...
	                        $fieldoldvideo = $langKey.'_old_video';
	                        $videoname = $langKey.'_video_file_name';
	                        $fieldvideolink = $langKey.'_video_link';
	                    @endphp
	            <input type="hidden" name="{{$fieldoldvideo}}" value="{{$module->$videoname}}">
	            <div class="form-group mb-2 {{ $errors->has($fieldvideo) ? 'has-error' : '' }}  @if($langKey!='en') {{'otherlangvideo'}} @endif" style="display: @if($langKey!='en' && old('same_video_for_all')=='1') {{'none'}} @else {{'block'}} @endif;">
	                ...
	                @endif
	                <p class="helper-block">
	                    <a href="{{$module->$fieldvideo->url()}}" target="_blank">{{$module->$videoname}}</a>
	                </p>
	            </div>
	            ...
	            @if(count($languages) > 0)
	                @foreach($languages as $langKey => $langValue)
	                    @php 
	                        ...
	                        $fieldoldslide = $langKey.'_old_slide';
	                        $slidename = $langKey.'_slide_file_name';
	                    @endphp
	            <input type="hidden" name="{{$fieldoldslide}}" value="{{$module->$slidename}}">
	            <div class="form-group mb-2 {{ $errors->has($fieldslide) ? 'has-error' : '' }}  @if($langKey!='en') {{'otherlangslide'}} @endif" style="display: @if($langKey!='en' && old('same_slide_for_all')=='1') {{'none'}} @else {{'block'}} @endif;">
	                ...
	                <input type="file" id="{{$fieldslide}}" name="{{$fieldslide}}" class="frm-field" value="">
	                ...
	                @endif
	                <p class="helper-block">
	                    <a href="{{$module->$fieldslide->url()}}" target="_blank">{{$module->$slidename}}</a>
	                </p>
	            </div>
